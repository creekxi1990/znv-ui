...
export default {
  ...
  render (h) {
    ...
    const view = h(this.tag, {
      class: ['znv-scrollbar__view', this.viewClass],
      style: this.viewStyle,
      ref: 'resize'
    }, this.$slots.default)
    // 创建容器
    const wrap = h(this.tag, {
      class: [this.wrapClass, 'znv-scrollbar__wrap', gutter ? '' : 'znv-scrollbar__wrap--hidden-default'],
      style: style,
      ref: 'wrap',
      on: {
    ...
    if (!this.normal) {
      ...
      nodes = [
        h(this.tag, {
          class: [this.wrapClass, 'znv-scrollbar__wrap'],
          style: style,
          ref: 'wrap'
        }, [view])
      ]
    }
    return h('div', { class: 'znv-scrollbar' }, nodes)
  },
  methods: {
    handleScroll () {
