    ...
    <div :class="classes" :style="styles" v-show="backTop" @click="back">
      <slot>
        <div :class="innerClasses">
          <znv-icon name="md-arrow-dropup" size="24"></znv-icon>
        </div>
      </slot>
    </div>
  </transition>
  ...
  export default {
    ...
    mounted() {
      ...
      if (parentScrollBox) {
        ...
        this.domEl = this.$parent.$el.className === 'znv-scrollbar'
          ? this.$parent.$el.querySelector('.znv-scrollbar__wrap')
          : window
      }
