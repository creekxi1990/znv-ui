  ...
  >
    <div ref="reference" :class="[prefixCls + '-rel']">
      <slot>
        ...
        >
          <znv-icon @click.native.stop.prevent="handleIconClick($event)" :name="arrowType" slot="suffix"/>
        </znv-input>
      </slot>
    </div>
...
<script>
  import ZnvInput from '../../components/input/input.vue'
  import Drop from '../../components/select/drop.vue'
  import ZnvIcon from '../../components/icon/icon.vue'
  ...
  const prefixCls = 'znv-date-picker'
  const pickerPrefixCls = 'znv-picker'
  ...
  const pulseElement = (el) => {
    const pulseClass = 'znv-date-picker-btn-pulse'
  ...
  export default {
    mixins: [Emitter],
    components: { ZnvInput, Drop, ZnvIcon },
    ...
    methods: {
      ...
      navigateDatePanel(direction, shift) {
        ...
        if (shift) {
          ...
          // pulse button
          const button = this.$refs.drop.$el.querySelector(`.znv-date-picker-${position}-btn-arrow${double}`)
      ...
      handleClear() {
        ...
        this.$emit('on-clear')
        this.dispatch('ZnvFormItem', 'on-form-change', '')
        this.emitChange(this.type)
        this.reset()
      ...
      emitChange(type) {
        this.$nextTick(() => {
          ...
          this.dispatch('ZnvFormItem', 'on-form-change', this.publicStringValue)
        })
      },
      parseDate(val) {
        const isRange = this.type.includes('range')
