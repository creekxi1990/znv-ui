<template>
  <div
    ...
    <span v-if="showText || showScore" class="znv-rate__text" :style="{ color: textColor }">{{ text }}</span>
  </div>
</template>
    ...
    methods: {
      ...
      setCurrentValue (value, event) {
        ...
        if (this.allowHalf) {
          ...
          if (hasClass(target, 'znv-rate__item')) {
            target = target.querySelector('.znv-rate__icon')
          }
          if (hasClass(target, 'znv-rate__decimal')) {
