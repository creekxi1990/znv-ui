...
<script>
  ...
  function getPropByPath(obj, path) {
    ...
    for (let len = keyArr.length; i < len - 1; ++i) {
      ...
      if (key in tempObj) {
        ...
        throw new Error('[znv-ui warn]: please transfer a valid prop path to form item!')
      }
    }
    return {
      o: tempObj,
  ...
  export default {
    ...
    mounted() {
      if (this.prop) {
        this.dispatch('ZnvForm', 'on-form-item-add', this)
    ...
    beforeDestroy() {
      this.dispatch('ZnvForm', 'on-form-item-remove', this)
    }
  }
</script>
