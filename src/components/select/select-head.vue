<template>
  <div @click="onHeaderClick" :class="headCls">
      <span :class="[prefixCls + '-prefix']" v-if="$slots.prefix || prefix">
        <slot name="prefix">
            <znv-icon :name="prefix" v-if="prefix"/>
        </slot>
      </span>
    <template v-for="(item, index) in selectedMultiple">
      <div class="znv-tag znv-tag-checked" v-if="maxTagCount === undefined || index < maxTagCount" :key="index">
        <span class="znv-tag-text">{{ item.label }}</span>
        <znv-icon name="ios-close" @click.native.stop="removeTag(item)"></znv-icon>
      </div>
    </template>
    <div class="znv-tag znv-tag-checked" v-if="maxTagCount !== undefined && selectedMultiple.length > maxTagCount">
      <span class="znv-tag-text znv-select-max-tag">
    ...
    <znv-icon name="ios-close-circle" :class="[prefixCls + '-arrow']" v-if="resetSelect"
            @click.native.stop="onClear"></znv-icon>
    <znv-icon name="ios-arrow-down" :class="[prefixCls + '-arrow']"
            v-if="!resetSelect && !remote"></znv-icon>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      removeTag(value) {
        ...
        this.dispatch('ZnvSelect', 'on-select-selected', value)
      },
      resetInputState() {
