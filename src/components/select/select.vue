...
<script>
  ...
  const optionRegexp = /^znv-option$/i
  const optionGroupRegexp = /znv-option-?group/i
    ...
    computed: {
      ...
      dropdownCls() {
        return {
          ...
          'znv-auto-complete': this.autoComplete,
    ...
    watch: {
      value(value) {
        ...
        if (value === '') {
          ...
          this.dispatch('ZnvFormItem', 'on-form-change', this.publicValue)
        }
      },
      values(now, before) {
