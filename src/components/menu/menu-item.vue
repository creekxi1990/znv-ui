  ...
  export default {
    ...
    methods: {
      handleClickItem (event, newWindow = false) {
        ...
        if (newWindow || this.target === '_blank') {
          ...
          let parentMenu = findComponentUpward(this, 'ZnvMenu')
          if (parentMenu) {
            parentMenu.handleEmitSelectEvent(this.name)
          }
        } else {
          let parent = findComponentUpward(this, 'ZnvSubmenu')

          if (parent) {
            this.dispatch('ZnvSubmenu', 'on-menu-item-select', this.name)
          } else {
            this.dispatch('ZnvMenu', 'on-menu-item-select', this.name)
          }
    ...
    mounted () {
      this.$on('on-update-active-name', (name) => {
        if (this.name === name) {
          this.active = true
          this.dispatch('ZnvSubmenu', 'on-update-active-name', name)
        } else {
          this.active = false
        }
      })
