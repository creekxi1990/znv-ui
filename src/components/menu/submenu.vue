<template>
  <li :class="classes" @mouseenter="handleMouseenter" @mouseleave="handleMouseleave">
    <div :class="[prefixCls + '-submenu-title']" ref="reference" @click.stop="handleClick" :style="titleStyle">
      ...
      <znv-icon :name="arrowName" :class="[prefixCls + '-submenu-title-icon']"/>
    </div>
    ...
    <transition name="zoom-in-top" v-else>
      <drop v-show="opened"
            :placement="placement"
            :class-name="`znv-menu-horizontal-dropdown ${rootMenu.theme}`"
            ref="drop"
            :style="dropStyle"
  ...
  export default {
      ...
      handleClick() {
        ...
        if (this.accordion) {
          this.$parent.$children.forEach(item => {
            if (item.$options.name === 'ZnvSubmenu') item.opened = false
          })
        }
        this.opened = !opened
        this.menu.updateOpenKeys(this.name)
    ...
    mounted() {
      this.$on('on-menu-item-select', (name) => {
        ...
        this.dispatch('ZnvMenu', 'on-menu-item-select', name)
        return true
      })
      this.$on('on-update-active-name', (status) => {
        if (findComponentUpward(this, 'ZnvSubmenu')) {
          this.dispatch('ZnvSubmenu', 'on-update-active-name', status)
        }
        if (findComponentsDownward(this, 'ZnvSubmenu')) {
