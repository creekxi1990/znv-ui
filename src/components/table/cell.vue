<template>
  <div :class="classes" ref="cell">
    ...
    <template v-if="renderType === 'selection'">
      <znv-checkbox :value="checked" @click.native.stop="handleClick" @on-change="toggleSelect"
                  :disabled="disabled"></znv-checkbox>
    </template>
    <template v-if="renderType === 'html'"><span v-html="row[column.key]"></span></template>
    <template v-if="renderType === 'normal'">
      <template v-if="column.tooltip && tableRoot.tooltipTheme">
        <znv-tooltip append-to-body :content="row[column.key]" :theme="tableRoot.tooltipTheme" :disabled="!showTooltip"
                   :max-width="300" class="znv-table-cell-tooltip">
                <span ref="content" @mouseenter="handleTooltipIn" @mouseleave="handleTooltipOut"
                      class="znv-table-cell-tooltip-content">{{ row[column.key] }}</span>
        </znv-tooltip>
      </template>
      <span v-else-if="column.tooltip && !tableRoot.tooltipTheme" class="znv-table-cell-tooltip-content"
