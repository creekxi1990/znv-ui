<template>
  <div :class="wrapClasses" :style="styles">
        ...
        <table cellspacing="0" cellpadding="0" border="0">
          ...
          <tr>
            <td :style="{'height':bodyStyle.height,'width':`${this.headerWidth}px`}">
              <znv-empty v-if="!data || data.length === 0">{{ noDataText }}</znv-empty>
            </td>
          </tr>
          </tbody>
        </table>
    ...
    <!--加载中-->
    <znv-loading fix size="large" v-if="loading">
...
//                    const width = this.bodyHeight === 0 ? this.tableWidth : this.tableWidth - this.scrollBarWidth;
    ...
    mounted() {
      ...
      if (this.draggable) {
        const table = this.$refs.tbody.$el.querySelector('.znv-table-tbody')
        const self = this
        Sortable.create(table, {
          animation: 150,
          ghostClass: 'znv-table-ghost-class',
