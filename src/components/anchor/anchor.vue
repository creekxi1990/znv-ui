<template>
  <div class="znv-anchor-wrapper">
    <div class="znv-anchor">
      <div class="znv-anchor-ink">
        <znv-icon v-if="icon" :name="icon" :style="iconStyle" :color="activeColorStr"></znv-icon>
        <span v-show="!icon&&showInk" class="znv-anchor-ink-ball"
              :style="{borderColor:activeColorStr, top: `${inkTop}px`}"></span>

        <span v-show="!icon&&!showInk" class="znv-anchor-ink-line"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateTitleOffset() {
        const links = findComponentsDownward(this, 'ZnvAnchorLink').map(link => {
          return link.href
        })
    ...
    mounted() {
      ...
      if (this.containerId) {
        ...
        const scroll = findComponentUpward(this, 'ZnvScrollbar')
        this.domEl = scroll ? scroll.$el.querySelector('.znv-scrollbar__wrap') : window
      }
